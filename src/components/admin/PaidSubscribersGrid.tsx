...
function sortRows(rows: PaidSubscriberRow[], sortKey: SortKey, sortDir: "asc" | "desc"): PaidSubscriberRow[] {
...
export function PaidSubscribersGrid() {
  const [rows, setRows] = useState<PaidSubscriberRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch("/api/admin/paid-subscribers", { cache: "no-store", credentials: "same-origin" });
      if (res.status === 403) {
        setLoadError("You do not have access.");
        return;
      }
      if (!res.ok) {
        setLoadError("Could not load paid subscribers.");
        return;
      }
      const data = (await res.json()) as { rows?: PaidSubscriberRow[] };
  ...
  async function removePaidUser(id: string) {
    if (!confirm("Permanently delete this user account? They will lose login and subscription access here.")) {
      return;
    }
    setRemovingId(id);
    try {
      const res = await fetch(`/api/admin/paid-subscribers/${encodeURIComponent(id)}`, {
        method: "DELETE",
          ...
          <thead className="sticky top-0 z-10 border-b border-[#6e0f1f]/45 bg-[#2a0a10] shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
            <tr className="border-b border-white/10">
              <th colSpan={5} className="px-3 py-1.5 text-right font-normal">
                <a
                  href="/api/admin/paid-subscribers/export"
                ...
                <tr key={r.id} className="border-b border-white/[0.08] bg-black transition hover:bg-[#0a0a0a]">
                  ...
                  <td className="px-3 py-2 text-right align-middle">
                    <button
                      ...
                      onClick={() => void removePaidUser(r.id)}
