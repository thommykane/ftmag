...
import { ITALY_REGION_STATE_SLUG } from "@/data/restaurants/italy78Seed";
...
export async function getItalyRestaurants(): Promise<RestaurantDTO[]> {
  const rows = await prisma.restaurant.findMany({
...
export async function getFilterOptions(): Promise<{
  ...
  const [cuisineNat, cuisineEu, cuisineIt, stateRows, countryNat, countryEu, countryIt] =
    await Promise.all([
      ...
      prisma.restaurant.findMany({
        where: { italyRank: { not: null }, cuisine: { not: "" } },
        select: { cuisine: true },
        distinct: ["cuisine"],
      }),
      prisma.restaurant.findMany({
      ...
      prisma.restaurant.findMany({
        where: { italyRank: { not: null }, country: { not: "" } },
        select: { country: true },
        distinct: ["country"],
      }),
    ]);
  ...
  const cuisines = Array.from(
    new Set([
      ...cuisineNat.map((r) => r.cuisine),
      ...cuisineEu.map((r) => r.cuisine),
      ...cuisineIt.map((r) => r.cuisine),
    ]),
  ).sort((a, b) => a.localeCompare(b));

  const countries = Array.from(
    new Set([
      ...countryNat.map((r) => r.country),
      ...countryEu.map((r) => r.country),
      ...countryIt.map((r) => r.country),
    ]),
  ).sort((a, b) => a.localeCompare(b));
...
/** Italy detail — `/top-restaurants/italy/[city]/[slug]` matches citySlug + nameSlug (state `italy`). */
export async function getRestaurantByItalyPath(
  citySlug: string,
  nameSlug: string,
): Promise<RestaurantDTO | null> {
  const row = await prisma.restaurant.findFirst({
