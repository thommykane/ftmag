...
/** Body: { ids: string[], scope?: "national" | "europe" | "italy" } — full new order for ranked entries (ranks 1..n). */
export async function PATCH(req: NextRequest) {
  ...
  const scope =
    body?.scope === "europe" ? "europe" : body?.scope === "italy" ? "italy" : "national";
  if (ids.length === 0) {
