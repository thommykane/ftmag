...
import { getRestaurantByItalyPath } from "@/lib/restaurants-queries";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const r = await getRestaurantByItalyPath(citySlug, nameSlug);
  if (!r) return { title: "Restaurant | Food & Travel Magazine" };
  return {
    title: `${r.name} | Top Restaurants | Food & Travel Magazine`,
    description: `${r.name} — ranked among Italy’s top restaurants.`,
  };
}

export default async function ItalyRestaurantDetailPage({ params }: Props) {
  const { city, slug } = await params;
  const nameSlug = decodeURIComponent(slug);
  const citySlug = decodeURIComponent(city);
  const r = await getRestaurantByItalyPath(citySlug, nameSlug);
